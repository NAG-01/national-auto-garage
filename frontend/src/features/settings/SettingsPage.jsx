...
export const SettingsPage = () => {
  const [mainTab, setMainTab] = useState('website'); // 'website' | 'system'
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto pb-20">
      <PageHeader
        title="Settings & Website CMS"
        subtitle="Visiting Website Control, Admin Login Credentials, Header Badges, and UI Customizations."
      />

      {/* Main Top Switcher Tabs */}
      <div className="flex items-center gap-3 p-1.5 rounded-2xl bg-slate-200/70 border border-slate-300/80 shadow-inner">
        <button
          type="button"
          ...
          className={`flex-1 py-3 px-4 rounded-xl text-xs sm:text-sm font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer ${
            mainTab === 'website'
              ? 'bg-white text-[#0284C7] shadow-md shadow-slate-900/5 scale-[1.01]'
              : 'text-slate-600 hover:text-slate-900 hover:bg-white/40'
          }`}
        >
          <Globe className="w-4 h-4" />
          <span>🌐 Visiting Website CMS</span>
        </button>
        ...
        >
          ...
          <span>⚙️ Admin & Portal Settings</span>
        </button>
      </div>

      {/* TAB 1: VISITING WEBSITE CMS */}
      {mainTab === 'website' && <WebsiteSettingsTab />}

      {/* TAB 2: PORTAL & SECURITY SETTINGS */}
      {mainTab === 'system' && (
            ...
            <Card className="p-6 sm:p-8 border border-slate-200 shadow-xs space-y-6 bg-white">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-slate-500 font-medium">
                      Login screen par yahi username daalna hoga.
                    </p>
                  </div>
              ...
              <div className="flex items-center justify-between gap-4 pt-4 border-t border-slate-200">
                ...
                <Button
                  type="submit"
                  disabled={savingAuth}
                  className="px-8 py-3 text-xs font-black bg-amber-600 hover:bg-amber-700 text-white shadow-lg shadow-amber-900/20 cursor-pointer"
                >
            ...
            <Card className="p-6 sm:p-8 border border-slate-200 shadow-xs space-y-6 bg-white">
              <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
                ...
                <div>
                  <h2 className="text-base font-black text-slate-900 tracking-tight">
                    Dynamic UI Badges & Portal Titles
                  </h2>
                  <p className="text-xs text-slate-500 font-medium mt-0.5">
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                  {['ADMIN', 'ADMIN PORTAL', 'WORKSHOP PORTAL', 'GARAGE ADMIN', 'SYSTEM PORTAL'].map((txt) => (
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-full text-xs font-extrabold bg-white hover:bg-[#0284C7] text-slate-700 hover:text-white border border-slate-200/90 hover:border-[#0284C7] transition-all shadow-2xs active:scale-95 cursor-pointer"
                    >
                      + "{txt}"
                    </button>
                  ))}
              ...
              <div className="flex items-center justify-between gap-4 pt-4 border-t border-slate-200">
                ...
                <Button
                  type="submit"
                  disabled={saving}
                  className="px-8 py-3 text-xs font-black bg-[#0284C7] hover:bg-[#0369A1] shadow-lg shadow-sky-900/20 cursor-pointer"
                >
