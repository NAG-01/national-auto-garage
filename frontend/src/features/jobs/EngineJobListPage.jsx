...
export const EngineJobListPage = () => {
  ...
  const fetchJobs = useCallback(async (isInitial = false) => {
    ...
    try {
      const res = await api.get('/jobs', {
        params: { search, serviceType: 'ENGINE_JOB', page, limit: 15 },
      });
      ...
      console.error('Failed to fetch engine jobs:', err);
      setError(err.message || 'Failed to load engine jobs.');
    } finally {
      setLoading(false);
      setFetching(false);
    }
  ...
  const handleDeleteJob = async () => {
    ...
    try {
      ...
      toast.success('Engine repair job deleted successfully.');
      fetchJobs(false);
      setDeleteDialogOpen(false);
      setDeleteTarget(null);
    } catch (err) {
      toast.error(err.message || 'Failed to delete engine job.');
    } finally {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Engine Jobs"
        subtitle="Engine kaam, customer detail aur complaint yahan manage karein."
      />
      ...
      <div className="min-h-[420px] relative">
        {loading ? (
          ...
          <EmptyState
            icon={Flame}
            title="No engine jobs found"
            description={
              search
                ? 'No engine jobs match your active search filter.'
                : 'Create your first engine repair job card to record customer bike details.'
            }
            actionText="New Engine Job"
            onAction={() => {
              setSelectedJob(null);
              setModalOpen(true);
            }}
            ...
            <Table>
              <TableHeader>
                <TableRow hover={false}>
                  ...
                  <TableHead>Problem / Engine Complaints</TableHead>
      ...
      <ConfirmDialog
        ...
        title={`Delete Engine Job #${deleteTargetSeq}?`}
        message={`Are you sure you want to delete engine job #${deleteTargetSeq} for '${deleteTarget?.customerNameSnapshot}'? Click OK to confirm.`}
